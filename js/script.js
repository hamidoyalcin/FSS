...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Leaflet Karteninitialisierung für die Standortkarte
  const mapElement = document.getElementById('map');
  // Prüfe, ob das Karten‑Element und die Leaflet‑Bibliothek verfügbar sind
  if (mapElement && window.L) {
    // Erzeuge eine Karte und zentriere sie auf den Koordinaten der Fahrschule
    const map = L.map('map').setView([48.3705, 10.90808], 17);
    // Füge die OSM‑Kacheln hinzu
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      attribution: '&copy; OpenStreetMap contributors'
    }).addTo(map);
    // Marker auf die Adresse setzen und mit Popup versehen
    L.marker([48.3705, 10.90808])
      .addTo(map)
      .bindPopup('Fahrschule Seyis');
  }
});
